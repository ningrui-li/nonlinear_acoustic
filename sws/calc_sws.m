function [sws] = calc_sws(focus, alpha, beta, depth, printPlot)
%% calc_sws.m
...
if printPlot
    figure
    plot(ttp*1e3, lat*1e3, 'k-') % note conversion from s to ms for time to peak
    ...
    title(sprintf(['SWS Estimation at Depth of %.1f mm', nln, ...
                   '(focal depth=%d mm, \\alpha=%.3f dB/cm/MHz, \\beta=%.1f)'],...
                   axial(depth), focus, alpha, beta), 'FontSize', 18)
               
    eval(sprintf('print -dpng sws_ttp_foc%d_a%.3f_B%.1f_depth%.1f.png', focus, alpha, beta, axial(depth)))
end
